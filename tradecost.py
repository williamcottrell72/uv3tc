...
def quote_exact_input_single(w3, token_in, token_out, fee, amount_in):
    ...
    amount_out, sqrt_after, ticks_crossed, gas_est = quoter.functions.quoteExactInputSingle(
        params
    ).call()
...
def check_pool_liquidity(w3, pool_addr):
    ...
    try:
        ...
        return {"initialized": True, "has_liquidity": True, "sqrt_price_x96": sqrt_price_x96}
    except Exception as e:
        return {"initialized": False, "has_liquidity": False, "error": str(e)}


def quote_with_price_impact(w3, token_in, token_out, fee, amount_in, pool_addr, decimals_in, decimals_out):
    """
    ...
    Returns:
        dict: Contains amountOut, execPrice, midPrice, priceImpactBps, ticksCrossed, gasEstimate
    ...
    if not liquidity_check.get("has_liquidity"):
        raise ValueError(f"Pool at {pool_addr} does not have sufficient liquidity or is not initialized")
    ...
    if token_in.lower() == token0.lower():
        ...
        mid_price = (1 / price_token1_per_token0_raw) * (10 ** decimals_in) / (10 ** decimals_out)
...
def analyze_trade_costs(w3, token_in_symbol, token_out_symbol, amounts_in, fee=None, verbose=True):
    """
    ...
    Returns:
        pandas.DataFrame with columns:
            ...
            - mid_price: Mid market price (token_out per token_in)
    ...
    pool_config = get_pool_config(token_in_symbol, token_out_symbol, fee, w3=w3, auto_discover=True)
    ...
    if verbose:
        ...
        print("Note: Low-liquidity pools may fail for larger trade sizes due to high price impact")
    ...
    for amount_in_human in amounts_in:
        ...
        try:
            ...
            if verbose:
                print(f"✓ ${amount_in_human:,.0f} {token_in_symbol} → "
            ...
            if verbose:
                ...
                if "SPL" in error_str or "Sqrt Price Limit" in error_str:
                    print(f"✗ ${amount_in_human:,.0f}: Trade too large for available liquidity (price impact too high)")

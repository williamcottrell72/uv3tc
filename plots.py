...
def plot_amount_in_vs_out(df, token_in_symbol, token_out_symbol):
    ...
    fig.add_trace(go.Scatter(
        ...
        marker=dict(size=8),
        hovertemplate='<b>Input:</b> %{x:,.0f} ' + token_in_symbol + '<br>' +
                      '<b>Actual Output:</b> %{y:.8f} ' + token_out_symbol + '<br>' +
                      '<extra></extra>'
    ))
    ...
    fig.add_trace(go.Scatter(
        ...
        hovertemplate='<b>Input:</b> %{x:,.0f} ' + token_in_symbol + '<br>' +
                      '<b>Ideal Output:</b> %{y:.8f} ' + token_out_symbol + '<br>' +
                      '<extra></extra>'
    ))
    ...
    fig.update_layout(
        ...
        yaxis=dict(
            title=f'Amount Out ({token_out_symbol})',
            gridcolor='lightgray'
        ),
...
def plot_price_impact(df, token_in_symbol, token_out_symbol):
    ...
    fig.add_trace(go.Scatter(
        ...
        marker=dict(size=8),
        hovertemplate='<b>Trade Size:</b> %{x:,.0f} ' + token_in_symbol + '<br>' +
    ...
    fig.update_layout(
        ...
        yaxis=dict(
            title='Price Impact (basis points)',
            gridcolor='lightgray'
        ),
        template='plotly_white',
        hovermode='closest',
...
def plot_cost_of_impact(df, token_in_symbol, token_out_symbol):
    ...
    fig.add_trace(go.Scatter(
        ...
        marker=dict(size=8),
        hovertemplate='<b>Trade Size:</b> %{x:,.0f} ' + token_in_symbol + '<br>' +
                      '<b>Lost:</b> %{y:.8f} ' + token_out_symbol + '<br>' +
    ...
    fig.update_layout(
        ...
        yaxis=dict(
            title=f'Tokens Lost ({token_out_symbol})',
            gridcolor='lightgray'
        ),
        template='plotly_white',
        hovermode='closest',
...
def plot_combined_analysis(df, token_in_symbol, token_out_symbol):
    ...
    # Actual Amount Out
    fig.add_trace(
        go.Scatter(x=df["amount_in"], y=df["amount_out"],
    ...
    # Ideal Amount Out
    fig.add_trace(
        go.Scatter(x=df["amount_in"], y=df["ideal_amount_out"],
                   mode='lines+markers', name='Ideal Output',
                   line=dict(color='#2ca02c', width=2, dash='dash'),
                   marker=dict(size=6, symbol='x'),
                   legendgroup='output'),
        row=1, col=1
    ...
    # Price Impact
    fig.add_trace(
        go.Scatter(x=df["amount_in"], y=df["price_impact_bps"],
    ...
    fig.add_trace(
        go.Scatter(x=df["amount_in"], y=df["exec_price_inv"],
                   mode='lines+markers', name='Exec Price',
    ...
    fig.add_trace(
        go.Scatter(x=df["amount_in"], y=df["mid_price_inv"],
    ...
    # Gas Cost in USD
    fig.add_trace(
        go.Scatter(x=df["amount_in"], y=df["gas_cost_usd"],
                   mode='lines+markers', name='Gas Cost',
    ...
    fig.update_xaxes(title_text=f"{token_in_symbol} Input", type="log", tickformat=",.0f", tickangle=-45, row=1, col=1)
    fig.update_xaxes(title_text=f"{token_in_symbol} Input", type="log", tickformat=",.0f", tickangle=-45, row=1, col=2)
    fig.update_xaxes(title_text=f"{token_in_symbol} Input", type="log", tickformat=",.0f", tickangle=-45, row=2, col=1)
    fig.update_xaxes(title_text=f"{token_in_symbol} Input", type="log", tickformat=",.0f", tickangle=-45, row=2, col=2)
...
def print_summary_stats(df, token_in_symbol, token_out_symbol, pool_address, fee):
    ...
    print(f"\nPrice Impact:")
    print(f"  Min: {df['price_impact_bps'].min():.2f} bps ({df['price_impact_pct'].min():.2f}%)")
    print(f"  Max: {df['price_impact_bps'].max():.2f} bps ({df['price_impact_pct'].max():.2f}%)")
    print(f"  Mean: {df['price_impact_bps'].mean():.2f} bps ({df['price_impact_pct'].mean():.2f}%)")
    print(f"\nExecution Price:")
    ...
    print(f"  Mid Price: {df['mid_price'].iloc[0]:.10f} {token_out_symbol}/{token_in_symbol}")
    print(f"  (Inverted) Min: {df['exec_price_inv'].min():.2f} {token_in_symbol}/{token_out_symbol}")
    print(f"  (Inverted) Max: {df['exec_price_inv'].max():.2f} {token_in_symbol}/{token_out_symbol}")
    print(f"  (Inverted) Mid Price: {df['mid_price_inv'].iloc[0]:.2f} {token_in_symbol}/{token_out_symbol}")
    print(f"\nCost of Impact:")
    ...
    print(f"  Mean: {df['gas_estimate'].mean():,.0f} gas (${df['gas_cost_usd'].mean():.2f})")
